  ...
  define(['jquery', 'compiled/calendar/commonEventFactory', 'compiled/calendar/TimeBlockList', 'jst/calendar/editCalendarEvent', 'jst/calendar/genericSelect', 'jquery.instructure_date_and_time', 'jquery.instructure_forms', 'jquery.instructure_misc_helpers', 'vendor/date'], function($, commonEventFactory, TimeBlockList, editCalendarEventTemplate, genericSelectTemplate) {
    ...
    return EditCalendarEventDetails = (function() {
      ...
      EditCalendarEventDetails.prototype.setupTimeAndDatePickers = function() {
        ...
        this.form.find(".time_field").time_field().blur(__bind(function(jsEvent) {
          var endDate, end_time, startDate, start_time;
          start_time = this.form.find(".time_field.start_time").next(".datetime_suggest").text();
          if (this.form.find(".time_field.start_time").next(".datetime_suggest").hasClass('invalid_datetime')) {
            start_time = null;
          }
          if (start_time == null) {
            start_time = this.form.find(".time_field.start_time").val();
          }
          end_time = this.form.find(".time_field.end_time").next(".datetime_suggest").text();
          if (this.form.find(".time_field.end_time").next(".datetime_suggest").hasClass('invalid_datetime')) {
            end_time = null;
          }
          if (end_time == null) {
            end_time = this.form.find(".time_field.end_time").val();
          }
          ...
          if (startDate) {
            this.form.find(".time_field.start_time").val(startDate.toString('h:mmtt').toLowerCase());
          }
          if (endDate) {
            return this.form.find(".time_field.end_time").val(endDate.toString('h:mmtt').toLowerCase());
          }
        }, this));
      ...
      EditCalendarEventDetails.prototype.formSubmit = function(jsEvent) {
        var data, end_date, newEvent, objectData, params, start_date, _ref, _ref2;
        jsEvent.preventDefault();
        ...
        params = {
          'calendar_event[title]': (_ref2 = data.title) != null ? _ref2 : this.event.title,
