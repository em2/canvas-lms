  ...
  def create
    ...
    if (errors_found)
      ...
      while (i < @course_titles.count)
        ...
        Delayed::Job.enqueue(RosterGenerateJob.new(@roster, @context, @probe, @instance, @stage, @course_title, @current_user, @num_students))
        #@roster.send_later(:generate_probes, @context, @probe, @instance, @stage, @course_title, @current_user, @num_students)
