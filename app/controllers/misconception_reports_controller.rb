class MisconceptionReportsController < ApplicationController
  ...
  def index
		if authorized_action(@context, @current_user, :read) && redirect_if_not_admin_or_teacher
			...
			add_crumb(t('#crumbs.misconception_reports', 'Misconception Reports'), named_context_url(@context, :context_quiz_misconception_reports_url, @quiz))
